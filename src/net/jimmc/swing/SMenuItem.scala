...
class SMenuItem(frame:SFrame, resPrefix:String)(action: =>Unit)
    ...
    if (toolPrompter!=null && !toolPrompt.isEmpty) {
        addFocusListener(new SMenuItemFocusListener())
        addMouseListener(new SMenuItemMouseListener())
    }
    ...
    class SMenuItemFocusListener() extends FocusListener {
    ...
    class SMenuItemMouseListener extends MouseAdapter {
